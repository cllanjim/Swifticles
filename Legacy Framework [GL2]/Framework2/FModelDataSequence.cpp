...
void FModelDataSequence::Load(const char *pFileBase, int pStartIndex, int pEndIndex, const char *pFileReferenceName)
{
    ...
    int aIndex=0;
    EnumList(FString, aString, aFileList)//aSequence.mList)
    {
...
void FModelDataSequence::Draw(float pFrame)
{
    if((mParent != 0) && (mCount > 0))
    {
        ...
        mDrawRenderer.mBindIndex = mBindIndex;
        mDrawRenderer.SetIndex(aIndex, aIndexCount);
        mDrawRenderer.SetData(aXYZ, aUVW, aNormal, aVertexCount);
        mDrawRenderer.Draw();
