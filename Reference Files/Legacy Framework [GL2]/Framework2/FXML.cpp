...
void FXML::Save(char *pFile)
{
    ...
    {
        FFile aFFile;
        aFFile.Write(mOutput, mOutputLength);
        aFFile.Save(pFile);
    }
}
...
void FXML::Load(char *pFile)
{
	...
	Parse((char*)aFFile.mData, aFFile.mLength);
}

void FXML::Parse(char *pData, int pLength)
{
    printf("Parsing XML: [%d] (%x)\n", pLength, pData);
...
BEGIN_STACK_LOOP:
	...
	//WE WILL ATTEMPT TO TREAT THIS LIKE AN FXML BODY TAG THING..
